...
def fillObsCPT(bayesNet, gameState):
    ...
    for house in gameState.getPossibleHouses():
        # for every house walls
        for obs in gameState.getHouseWalls(house):
            obsVar = OBS_VAR_TEMPLATE % obs  # obs(x,y)
            ...
            for assignment in obsFactor.getAllPossibleAssignmentDicts():
                # is food
                if assignment[FOOD_HOUSE_VAR] == BOTTOM_LEFT_VAL and house == bottomLeftPos or \
                    assignment[FOOD_HOUSE_VAR] == BOTTOM_RIGHT_VAL and house == bottomRightPos or \
                    assignment[FOOD_HOUSE_VAR] == TOP_LEFT_VAL and house == topLeftPos or \
                    assignment[FOOD_HOUSE_VAR] == TOP_RIGHT_VAL and house == topRightPos:
                    if assignment[obsVar] == NO_OBS_VAL: # None
                        prob = 0
                    elif assignment[obsVar] == RED_OBS_VAL: # Red
                        prob = PROB_FOOD_RED
                    else: # Blue
                        prob = 1-PROB_FOOD_RED
                # is ghost
                elif assignment[GHOST_HOUSE_VAR] == BOTTOM_LEFT_VAL and house == bottomLeftPos or \
                    assignment[GHOST_HOUSE_VAR] == BOTTOM_RIGHT_VAL and house == bottomRightPos or \
                    assignment[GHOST_HOUSE_VAR] == TOP_LEFT_VAL and house == topLeftPos or \
                    assignment[GHOST_HOUSE_VAR] == TOP_RIGHT_VAL and house == topRightPos:
                    if assignment[obsVar] == NO_OBS_VAL: # None
                        prob = 0
                    elif assignment[obsVar] == RED_OBS_VAL: # Red
                        prob = PROB_GHOST_RED
...
def getMostLikelyFoodHousePosition(evidence, bayesNet, eliminationOrder):
    ...
    best_prob = 0
    for assignment in foodmarginal.getAllPossibleAssignmentDicts():
        # find the best assignment by comparing the each one's prob
        if foodmarginal.getProbability(assignment) > best_prob:
            best_prob = foodmarginal.getProbability(assignment)
