...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet, queryVariables, evidenceDict, eliminationOrder):
        ...
        "*** YOUR CODE HERE ***"
        # form a problem 
        problem = bayesNet.getAllCPTsWithEvidence(evidenceDict)
        for i in eliminationOrder:
            # join
            problem, join = joinFactorsByVariable(problem, i)
            unconditionednum = len(join.unconditionedVariables())
            if unconditionednum > 1: # if only one , it doesn't need VE
                problem.append(eliminate(join, i))
        return normalize(joinFactors(problem))
